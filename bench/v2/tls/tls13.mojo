...
struct Timer:
    ...
    fn stop(self):
        ...
        print("    [TIMER] " + self.name + ": " + String(end - self.start) + "s")
...
struct TLS13Client[T: TLSTransport](Movable):
    ...
    fn encrypt_record(
        ...
        for b in sealed[1]:
            out.append(b)
        var res = build_record(CONTENT_APPDATA, out)
        t.stop()
        return res^
    ...
    fn perform_handshake(mut self) raises -> Bool:
        var t_total = Timer("perform_handshake_total")
        var t_prep = Timer("handshake_prep")
        ...
        var t_sh = Timer("read_server_hello")
        ...
        var t_enc_hs = Timer("read_encrypted_handshake")
        ...
        while not got_finished:
            ...
            while hc.remaining() >= 4:
                ...
                if mt == HS_CERTIFICATE:
                    var t_cert = Timer("handle_certificate")
                    for b in full:
                        self.transcript.append(b)
                    ...
                    if len(cert_list) > 0:
                        ...
                        var t_verify_chain = Timer("verify_chain")
                    ...
                    var t_cv = Timer("handle_cert_verify")
                    ...
                    if sig_alg == SIG_RSA_PSS_RSAE_SHA256:
                        var t_rsa = Timer("verify_rsa_pss")
                    ...
                    var t_fin = Timer("handle_finished")
        ...
        var t_final = Timer("handshake_final")
