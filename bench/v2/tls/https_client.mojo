...
struct HTTPSClient:
    ...
    fn do(mut self, var request: HTTPRequest) raises -> HTTPResponse:
        ...
        while not _has_header_terminator(initial):
            loop_count += 1
            if loop_count % 100 == 0:
                print("    [HTTP-DEBUG] Header read loop count: " + String(loop_count) + ", len=" + String(len(initial)))
            var more = Bytes(capacity=default_buffer_size)
            var n = conn.read(more)
            if n == 0:
                break
            initial += more.copy()
        print("    [HTTP-DEBUG] Header read loop finished, count=" + String(loop_count) + ", total_len=" + String(len(initial)))
