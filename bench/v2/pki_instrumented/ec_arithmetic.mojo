...
struct Timer:
    ...
    fn stop(self):
        ...
        print("    [MATH-TIMER] " + self.name + ": " + String(end - self.start) + "s")
...
@always_inline
fn add_mod[N: Int](a: UIntLimbs[N], b: UIntLimbs[N], m: UIntLimbs[N]) -> UIntLimbs[N]:
    var res = UIntLimbs[N]()
    var carry = UInt64(0)
...
@always_inline
fn sub_mod[N: Int](a: UIntLimbs[N], b: UIntLimbs[N], m: UIntLimbs[N]) -> UIntLimbs[N]:
    if cmp(a, b) >= 0:
...
fn mont_mul[N: Int](a: UIntLimbs[N], b: UIntLimbs[N], m: UIntLimbs[N], n0_inv: UInt64) -> UIntLimbs[N]:
    # CIOS Montgomery Multiplication
    var t_len = 2 * N + 2
    var t = UnsafePointer[UInt64].alloc(t_len)
    for i in range(t_len): t[i] = 0
...
fn mont_sqr[N: Int](a: UIntLimbs[N], m: UIntLimbs[N], n0_inv: UInt64) -> UIntLimbs[N]:
    return mont_mul(a, a, m, n0_inv)

fn mont_pow[N: Int](base: UIntLimbs[N], exp: UIntLimbs[N], m: UIntLimbs[N], n0_inv: UInt64, one_mont: UIntLimbs[N]) -> UIntLimbs[N]:
    var t = Timer("mont_pow N=" + String(N))
    var res = one_mont
    var b = base
...
struct FieldContext[N: Int](Copyable, Movable, ImplicitlyCopyable):
    var m: UIntLimbs[N]
    var n0_inv: UInt64
    var r2: UIntLimbs[N]
    var one: UIntLimbs[N]
    
    fn __init__(out self, m: UIntLimbs[N], n0_inv: UInt64, r2: UIntLimbs[N], one: UIntLimbs[N]):
        self.m = m
        self.n0_inv = n0_inv
        self.r2 = r2
        self.one = one
...
struct PointJac[N: Int](Copyable, Movable, ImplicitlyCopyable):
    var x: UIntLimbs[N]
    var y: UIntLimbs[N]
    var z: UIntLimbs[N]
...
fn from_affine[N: Int](x: UIntLimbs[N], y: UIntLimbs[N], ctx: FieldContext[N]) -> PointJac[N]:
    var z = ctx.one
...
fn jac_add[N: Int](p: PointJac[N], q: PointJac[N], ctx: FieldContext[N]) -> PointJac[N]:
    if p.is_infinity():
        return q
    if q.is_infinity():
        return p
...
fn precompute_table[N: Int](p: PointJac[N], ctx: FieldContext[N]) -> InlineArray[PointJac[N], 16]:
...
fn double_scalar_mul_windowed[N: Int](
    u1: UIntLimbs[N], p1: PointJac[N], u2: UIntLimbs[N], p2: PointJac[N], ctx: FieldContext[N]
) -> PointJac[N]:
    var t_total = Timer("double_scalar_mul_windowed N=" + String(N))
    var t_pre = Timer("precompute_tables")
    ...
    var t_loop = Timer("windowed_loop")
...
fn verify_generic[N: Int](
    ...
    print("    [DEBUG] verify_generic called for N=" + String(N))
