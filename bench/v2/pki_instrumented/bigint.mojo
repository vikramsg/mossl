...
fn montgomery_reduce(
    ...
    while i < k:
        ...
        while j < k:
            var idx = i + j
            var prod = UInt128(m) * UInt128(mod[j]) + UInt128(t[idx]) + carry
        ...
        while carry2 > 0:
            if idx3 >= len(t):
                t.append(UInt64(carry2))
                carry2 = 0
            else:
                var sum2 = UInt128(t[idx3]) + carry2
                t[idx3] = UInt64(sum2 & 0xFFFFFFFFFFFFFFFF)
                carry2 = sum2 >> 64
                idx3 += 1
        i += 1
    var out = List[UInt64]()
