...
fn sha256(data: Span[UInt8]) raises -> InlineArray[UInt8, 32]:
    ...
    for b in range(0, len(padded), 64):
        ...
        for i in range(16, 64):
            var s0 = (
                rotate_bits_right(w[i - 15], 7)
                ^ rotate_bits_right(w[i - 15], 18)
                ^ (w[i - 15] >> 3)
            )
            var s1 = (
                rotate_bits_right(w[i - 2], 17)
                ^ rotate_bits_right(w[i - 2], 19)
                ^ (w[i - 2] >> 10)
            )
        ...
        for i in range(64):
            var S1 = (
                rotate_bits_right(e, 6)
                ^ rotate_bits_right(e, 11)
                ^ rotate_bits_right(e, 25)
            )
            var ch = (e & f) ^ ((~e) & g)
            var temp1 = h_v + S1 + ch + k[i] + w[i]
            var S0 = (
                rotate_bits_right(a, 2)
                ^ rotate_bits_right(a, 13)
                ^ rotate_bits_right(a, 22)
            )
...
# Compatibility shim
fn sha256_bytes(data: List[UInt8]) -> List[UInt8]:
