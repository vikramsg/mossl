...
from pki.bigint import BigInt, mod_pow, add_mod, add_limbs, sub_limbs, mul_limbs, mod_inv, mod_mul
from crypto.bytes import bytes_to_hex, hex_to_bytes

fn test_large_math() raises:
    print("Testing multi-limb math...")
    # (2^64 - 1) + 1 = 2^64
    var a = List[UInt64](); a.append(0xffffffffffffffff)
    var b = List[UInt64](); b.append(1)
    ...
    # 2^64 - 1 = 2^64 - 1
    var c = List[UInt64](); c.append(0); c.append(1)
    var d = List[UInt64](); d.append(1)
    res = sub_limbs(c, d)
    assert_equal(len(res), 1)
    assert_equal(res[0], 0xffffffffffffffff)
    
    # (2^64) * (2^64) = 2^128
    var e = List[UInt64](); e.append(0); e.append(1)
    res = mul_limbs(e, e)
...
fn test_384bit_add_sub() raises:
    ...
    var one = List[UInt64](); one.append(1)
...
fn test_mod_pow_basic() raises:
    ...
    var base = BigInt(List[UInt8](3)).limbs
    var exp = BigInt(List[UInt8](4)).limbs
    var mod = BigInt(List[UInt8](10)).limbs
    var res = mod_pow(base, exp, mod)
    assert_equal(len(res), 1)
    assert_equal(res[0], 1)

fn test_mod_pow_large() raises:
    var base = BigInt(List[UInt8](7)).limbs
    var exp = BigInt(List[UInt8](13)).limbs
    var mod = BigInt(List[UInt8](11)).limbs
    var res = mod_pow(base, exp, mod)
    assert_equal(len(res), 1)
...
fn test_add_mod() raises:
    # 1 + 1 mod 3 = 2
    var a = BigInt(List[UInt8](1)).limbs
    var b = BigInt(List[UInt8](1)).limbs
    var mod = BigInt(List[UInt8](3)).limbs
...
fn test_mod_inv() raises:
    ...
    # 2^-1 mod 5 = 3
    var a = BigInt(List[UInt8](2)).limbs
    var mod = BigInt(List[UInt8](5)).limbs
...
fn test_large_mod_inv() raises:
    ...
    var p = BigInt.from_be_bytes(hex_to_bytes("fffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffeffffffff0000000000000000ffffffff"))

    var den = BigInt.from_be_bytes(hex_to_bytes("6c2fbc952c4c58debb3d317f2525b853f1e83b7a513428f9d3b462276be1718014c1639c3afd033af4863af921d41cbe"))

    

    # Expected den^-1 mod p

    var expected_den_inv = "4514732855333432276d4a1605b115e723d6ca59a9616f5053ac78017bf8514b118698d3f08f50b5a20f87cbd74aaa8f"

    

    var den_inv = mod_inv(den.limbs, p.limbs)

    assert_equal(bytes_to_hex(BigInt(den_inv).to_be_bytes(48)), expected_den_inv)
...
from pki.bigint import BigInt, mod_pow, add_mod, add_limbs, sub_limbs, mul_limbs, mod_inv, mod_mul, mod_reduce, shift_left
...
fn test_mod_reduce() raises:
    ...
    n = List[UInt64](); n.append(0); n.append(1)
