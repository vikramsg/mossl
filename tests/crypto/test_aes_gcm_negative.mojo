...
fn test_aes_gcm_negative() raises:
    ...
    assert_false(res[1], "Opened with corrupted tag")

    # 2. Corrupt ciphertext
    var bad_ct = ct.copy()
    bad_ct[0] ^= 1
    res = aes_gcm_open(key, iv, aad, bad_ct, tag)
    assert_false(res[1], "Opened with corrupted ciphertext")

    # 3. Wrong IV
    var bad_iv = iv.copy()
    bad_iv[0] ^= 1
    res = aes_gcm_open(key, bad_iv, aad, ct, tag)
    assert_false(res[1], "Opened with wrong IV")

    # 4. Wrong AAD
    var bad_aad = List[UInt8](9, 9, 9)
    res = aes_gcm_open(key, iv, bad_aad, ct, tag)
    assert_false(res[1], "Opened with wrong AAD")
